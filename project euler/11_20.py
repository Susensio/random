...
def p11():
    ...
    for x in range(20):
        for y in range(16):
            ...
            maximum = max((maximum, horizontal, vertical))
            if x < 16:
                diagonal1 = grid[x][y] * grid[x + 1][y + 1] * grid[x + 2][y + 2] * grid[x + 3][y + 3]
                diagonal2 = grid[x + 4][y] * grid[x + 4 - 1][y + 1] * grid[x + 4 - 2][y + 2] * grid[x + 4 - 3][y + 3]
                maximum = max((maximum, diagonal1, diagonal2))
    return maximum
...
def p12():
    ...
    return triangular
    # BRUTE FORCE, there are better ways (4s!)
...
def p15():
    ...
    def lattice_paths(width, height):
        if width == 0 or height == 0:
            ...
            return lattice_paths(width - 1, height) + lattice_paths(width, height - 1)
    # Recursive approach, impossible without memoization

    def lattice_paths_binomial(width, height):
        def binomial_coef(n, k):
            from math import factorial
            return factorial(n) // (factorial(k) * factorial(n - k))
        return binomial_coef(width + height, width)
...
def p18():
    ...
    @memoize
    def search_paths(row, col):
        value = triangle[row][col]
        if row < 14:
            return max(value + search_paths(row + 1, col),
                       value + search_paths(row + 1, col + 1))
        else:
            return value
...
def p19():
    ...
    * A leap year occurs on any year evenly divisible by 4, 
    ...
    def months_lenght(year):
        months = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
        if is_leap(year):
            months[1] += 1
        return tuple(months)

    day = (1 + (0 if start == 1900 else sum([sum(months_lenght(year)) for year in range(1900, start)]))) % 7
    sundays = 0
    for year in range(start, end + 1):
        for month in months_lenght(year):
            if day == 0:
                sundays += 1
            day = (day + month) % 7
